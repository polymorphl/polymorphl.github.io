...
def create_sprite_for_directory(source_dir: str, output_file: str) -> None:
    """Generate an SVG sprite from all .svg files in a directory."""
    # Ensure output directory exists
    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    if not os.path.exists(source_dir):
        print(f"Directory {source_dir} does not exist. Please create it and add .svg files.")
        return

    sprite_content = ['<svg xmlns="http://www.w3.org/2000/svg" style="display: none;">']

    # Get all SVG files except the sprite.svg itself
    files = [f for f in os.listdir(source_dir) if f.endswith('.svg') and f != 'sprite.svg']
    ...
    for filename in files:
        ...
        symbol = f'<symbol id="{symbol_id}" viewBox="{viewbox}">{content}</symbol>'
...
def create_sprites() -> None:
    ...
    icons_output = os.path.join(icons_dir, "sprite.svg")
    create_sprite_for_directory(icons_dir, icons_output)

    # Companies sprite
    companies_dir = os.path.join(project_root, "public", "assets", "companies")
